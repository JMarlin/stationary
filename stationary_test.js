...
function getEmptyTableSlot() {
  ...
  try {
    ...
    if (!(err$0 instanceof RangeError)) {
      throw err$0;
    }
...
var wasmBinaryFile;
wasmBinaryFile = "stationary_test.wasm";
...
var ASM_CONSTS = {24204:function() {
  ...
  var resizer = function(e) {
    ...
    if (display_ratio < window_ratio) {
      ...
      new_canvas.style.height = "" + window.innerHeight + "px";
      var canvas_width = window.innerHeight * display_ratio;
      new_canvas.style.width = "" + canvas_width + "px";
      new_canvas.style.left = "" + (window.innerWidth - canvas_width) / 2 + "px";
    } else {
      new_canvas.style.left = "0px";
      new_canvas.style.width = "" + window.innerWidth + "px";
      var canvas_height = window.innerWidth / display_ratio;
      new_canvas.style.height = "" + canvas_height + "px";
      new_canvas.style.top = "" + (window.innerHeight - canvas_height) / 2 + "px";
    }
  };
  ...
  var i = 0;
  var src_view = Module.HEAPU8.subarray(src_addr, src_addr + src_w * src_h * (src_depth === 0 ? 2 : 3));
  var i = 0;
  if (src_depth === 0) {
    for (var x = 0; x < display_w; x++) {
      for (var y = 0; y < display_h; y++) {
        ...
        var r = px0 & 31;
        var g = ((px1 & 3) << 3) + ((px0 & 224) >> 5);
        var b = (px1 & 124) >> 2;
        fb[y * 4 * display_w + x * 4 + 0] = (r << 3) + (r & 7);
        fb[y * 4 * display_w + x * 4 + 1] = (g << 3) + (g & 7);
        fb[y * 4 * display_w + x * 4 + 2] = (b << 3) + (b & 7);
        fb[y * 4 * display_w + x * 4 + 3] = 255;
      }
    }
  } else {
    for (var x = 0; x < display_w; x++) {
...
var asmLibraryArg = {"__syscall_fcntl64":___syscall_fcntl64, "__syscall_fstat64":___syscall_fstat64, "__syscall_fstatat64":___syscall_fstatat64, "__syscall_ioctl":___syscall_ioctl, "__syscall_lstat64":___syscall_lstat64, "__syscall_open":___syscall_open, "__syscall_stat64":___syscall_stat64, "emscripten_asm_const_int":_emscripten_asm_const_int, "emscripten_memcpy_big":_emscripten_memcpy_big, "emscripten_resize_heap":_emscripten_resize_heap, "environ_get":_environ_get, "environ_sizes_get":_environ_sizes_get, 
"fd_close":_fd_close, "fd_read":_fd_read, "fd_seek":_fd_seek, "fd_write":_fd_write, "gettimeofday":_gettimeofday, "setTempRet0":_setTempRet0};
var asm = createWasm();
var ___wasm_call_ctors = Module["___wasm_call_ctors"] = createExportWrapper("__wasm_call_ctors");
var _main = Module["_main"] = createExportWrapper("main");
var _do_render_loop_proc = Module["_do_render_loop_proc"] = createExportWrapper("do_render_loop_proc");
var _do_event_proc = Module["_do_event_proc"] = createExportWrapper("do_event_proc");
