...
var STATIC_BASE = 1024,
    ...
    STACKTOP = STACK_BASE,
    STACK_MAX = 32560,
    DYNAMIC_BASE = 5275440,
    DYNAMICTOP_PTR = 32400;
...
var wasmBinaryFile = 'stationary_test.wasm';
...
var ASM_CONSTS = {
  1745: function() {Module.do_render_proc = Module.cwrap('do_render_loop_proc');},  
 1806: function() {if(!!Module.render_interval) { return; } Module.render_interval = setInterval(function() { Module.do_render_proc(); }, 0);},  
 1929: function() {if(!Module.display_list) { Module.display_list = []; } return Module.display_list.length},  
 2022: function($0, $1) {var new_canvas = document.createElement('canvas'); document.body.style.margin = '0px'; document.body.style.backgroundColor = '#000'; var resizer = (e) => { var display_ratio = 320.0/240.0; var window_ratio = window.innerWidth / window.innerHeight; if(display_ratio < window_ratio) { new_canvas.style.top = "0px"; new_canvas.style.height = "" + window.innerHeight + "px"; var canvas_width = (window.innerHeight * display_ratio); new_canvas.style.width = "" + canvas_width + "px"; new_canvas.style.left = "" + ((window.innerWidth - canvas_width) / 2) + "px"; } else { new_canvas.style.left = "0px"; new_canvas.style.width = "" + window.innerWidth + "px"; var canvas_height = (window.innerWidth / display_ratio); new_canvas.style.height = "" + canvas_height + "px"; new_canvas.style.top = "" + ((window.innerHeight - canvas_height) / 2) + "px"; } }; window.addEventListener('resize', resizer); new_canvas.style.cursor = 'none'; new_canvas.style.position = 'absolute'; new_canvas.width = $0; new_canvas.height = $1; document.body.appendChild(new_canvas); Module.display_list.push({ canvas: new_canvas, framebuf: new Uint8Array(320 * 240 * 4) }); resizer(null);},  
 3183: function($0, $1, $2, $3, $4, $5, $6) {var display_id = $0; var display_w = $1; var display_h = $2; var src_w = $3; var src_h = $4; var src_addr = $5; var src_depth = $6; var disp_data = Module.display_list[display_id]; var canvas = disp_data.canvas; var fb = disp_data.framebuf; var ctx = canvas.getContext('2d'); var canv_data = ctx.getImageData(0, 0, display_w, display_h); var i = 0; var src_view = Module.HEAPU8.subarray( src_addr, src_addr + (src_w * src_h * (src_depth === 0 ? 2 : 3)) ); var i = 0; if(src_depth === 0) { for(var x = 0; x < display_w; x++) { for(var y = 0; y < display_h; y++) { var px0 = src_view[y * 2 * src_w + x * 2 + 0]; var px1 = src_view[y * 2 * src_w + x * 2 + 1]; var r = px0 & 0x1F; var g = ((px1 & 0x03) << 3) + ((px0 & 0xE0)>>5); var b = (px1 & 0x7C) >> 2; fb[y * 4 * display_w + x * 4 + 0] = (r << 3) + (r & 0x07); fb[y * 4 * display_w + x * 4 + 1] = (g << 3) + (g & 0x07); fb[y * 4 * display_w + x * 4 + 2] = (b << 3) + (b & 0x07); fb[y * 4 * display_w + x * 4 + 3] = 0xFF; } } } else { for(var x = 0; x < display_w; x++) { for(var y = 0; y < display_h; y++) { fb[y * 4 * display_w + x * 4 + 0] = src_view[y * 3 * src_w + x * 3 + 0]; fb[y * 4 * display_w + x * 4 + 1] = src_view[y * 3 * src_w + x * 3 + 1]; fb[y * 4 * display_w + x * 4 + 2] = src_view[y * 3 * src_w + x * 3 + 2]; fb[y * 4 * display_w + x * 4 + 3] = 0xFF; } } } canv_data.data.set(fb); ctx.putImageData(canv_data, 0, 0);}
};
...
/** @type {function(...*):?} */
var _do_render_loop_proc = Module["_do_render_loop_proc"] = createExportWrapper("do_render_loop_proc");
